# Generated from getting_started.md on 2026-04-19 12:36:21.079641
...
<p>The first time you launch it, you should open a <code>base directory</code> containing multiple git repositories. 
You can do this from the menu <code>File &gt; Open base directory</code> or by clicking on the <code>Open directory</code> icon in 
...
<li>The modified status: <code>OK</code> for no files modified, or number of modified files and/or files in conflict</li>
<li><p>The push/pull status: when on a branch with a remote tracking</p>

<ul>
<li><code>Up-to-date</code> if you have nothing to pull or push</li>
<li><code>2 to pull</code> if you are 2 commits behind the remote,</li>
<li><code>3 to push</code> if you are 3 commits ahead of the remote</li>
<li><code>1 to push, 2 to pull</code> if you are 1 commit ahead of the remote, and 2 commits behind</li>
</ul>
...
<p>If you remove or add repositories on the filesystem, you must refresh the list with <code>View &gt; Refresh local view</code> or with <code>F5</code> for Multigit to detect the changes.</p>
...
<p>Some of the columns of the repository list are optional. You can choose whether to show SHA1 and remot URL, from the menu <code>View &gt; Show &gt; Show Columns</code> or by right-clicking on the column headers and choosing which item to show.</p>

<p>You can also decide to hide the bottom panels, from the menu <code>View &gt; Show &gt; Show last commit panel</code> and 
<code>Show modified files panel</code>.</p>
...
You can do it for the repository list or for selected repositories with <code>View &gt; Refresh local view</code> or with <code>F5</code>. Refreshing the local view will
...
<p>Multigit supports multiple tabs, each with its own base directory. You can open a new tab with <code>View &gt; New tab</code> or with the right click
menu on the tab header. Select then a new
base directory for that tab. You can switch between tabs with <code>Ctrl+Tab</code> and <code>Ctrl+Shift+Tab</code>. Other tabs operations
...
<p>Tab operations are available from the menu <code>View</code> and from the right-click menu on the tab header.</p>
...
on the column <code>Status</code>), and commit them in one click. Or you can select all the repositories on a given branch (by sorting on the HEAD), 
and switch them to another branch in one click.</p>

<p>The possible operations are:</p>

<ul>
<li>Open an explorer on the repository with <code>View &gt; Show in explorer</code></li>
...
or fetching all repositories of all opened tabls. Check the first entries in the menu <code>Git</code> and the associated shortcuts. It is also
...
If you feel that an important operation should be present though, please open an issue to request it. In the meantime, you can use the <code>Custom git command</code> operation to run any git command on the selected repositories.</p>
...
<p>Multigit normally detects the git GUIs installed on your system. If it is not detected, you can configure it in the <code>File &gt; Edit settings &gt; External programs</code> menu. </p>
...
Configuring the double-click action is done in the <code>File &gt; Edit settings &gt; Double-click</code> menu.</p>
...
show the number and list of targeted repositories, and includes a button <code>Adjust repository list</code> to change this list.</p>

<p>The <code>Adjust repository list</code> dialog is composed of three parts:</p>
...
<ul>
<li><code>Retry</code> to retry the failed operation. Useful when the failure is due to a temporary issue</li>
<li><code>Continue</code> to continue the next operations in the sequence</li>
<li><code>Abort</code> to stop all the next operations on that repository</li>
</ul>

<p>An operation failing on one repository does not impact the operations running on the other repositories.</p>

<p>The right-click menu on a repository lets you copy the details of the operations on the clipboard. The <code>Copy log</code> button at the bottom lets you copy the full logs of all repositories to the clipboard.</p>
...
<p>This menu is available from the right-click menu on the repository list, and from the window top menu <code>View &gt; Copy</code> submenu. </p>

<p>You can also request the display of all git information with the <code>Git properties</code> operation, from the <code>Git</code> menu.</p>
...
<p>Using the menu <code>File &gt; Export to multigit file</code>, you can export the state of all repositories in the current tab to a multigit file. The
...
<p>In both cases, the exported file a JSON file with the <code>.mgit</code> extension, containing all your repositories. The next paragraphs 
...
This is where you need a multigit file, exported as a <em>project</em> . Use the menu <code>File &gt; Clone from multigit file</code> to do this.</p>
...
adjusted, especially if you are cloning with <code>ssh</code> protocol. You provide this information in the middle-left side part of the dialog.</p>
...
also lets choose <code>skip directory</code>, meaning that this particular repository will not be cloned. The last option, <code>delete directory</code> must be used
...
<p>You can also export all your repositories states to a CSV file. Just use <code>File &gt; Export to CSV</code>.  It will contain one line per 
...
<p>When you want to reproduce an exact repository state from a multigit file, you use the menu <code>File &gt; Apply a multigit file</code> . This will go
...
<p>To improve this situation, you can limit the maximum number of parallel git tasks running. In the <code>File &gt; Settings</code> dialog,
edit the number of parallel running git processes from <code>unlimited</code> to  a reasonable amount, like 10.</p>
